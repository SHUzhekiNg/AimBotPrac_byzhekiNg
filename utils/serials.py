...
import cv2 as cv
class RoboSerial():
    def __init__(self,com = "/dev/ttyUSB0",port = 115200,imgW = 640,imgH = 480):#Linux com1
        self.serial = serial.Serial(com,port,timeout=0.5)
        self.imgH,self.imgW = imgH,imgW
        self.cloud_value = None
    def calc_value(self,detected_xy,delta_time):
        ...
        de_x,de_y = de_xy
        de_x = int(de_x)
        de_y = self.imgH-int(de_y)
        delay_time=int(delta_time)
        ...
        cloud_value.append(delay_time>>8&0xff)
        cloud_value.append((delay_time)&0xff)
        sum = sum + (delay_time>>8&0xff)
        sum = sum + (delay_time&0xff)
        #cloud_value.append(delay_time)
    ...
    def send(self,pred_center,delta_time):
        assert self.serial is not None
        if pred_center is None:
            print("pred_center err")
            return
        self.calc_value(pred_center,delta_time)
        print(pred_center)
...
class RoboSerial_1():
    def __init__(self,com = "/dev/ttyUSB1",port = 115200,imgW = 640,imgH = 480):#Linux com1
        self.serial = serial.Serial(com,port,timeout=0.5)
        self.imgH,self.imgW = imgH,imgW
        self.cloud_value = None
    def calc_value(self,detected_xy,delta_time):
